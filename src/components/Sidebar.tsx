          ...
          <div className="p-4">
            ...
            <div className="mt-4 space-y-3">
              <Accordion type="single" collapsible className="w-full space-y-3">
                {sortedCountries.map(country => <AccordionItem key={country.name} value={country.name} className="border border-amber-100 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-all bg-white">
                  ...
                  navigateToLocation(country.lat, country.lng);
                    ...
                    <AccordionContent className="bg-gradient-to-b from-amber-50/30 to-white px-3 py-2">
                      <div className="space-y-2">
                        {country.cities.map(city => <div key={city.name} className="mb-3">
                            <Button variant="ghost" className="w-full justify-start text-sm p-2 h-auto rounded-md bg-white hover:bg-amber-50 border border-gray-100 hover:border-amber-200 transition-all shadow-sm" onClick={() => {
                        ...
                        if (isMobile) {
                          setTimeout(() => setSelectedLocation({
                            ...
                            {selectedLocation && selectedLocation.name === city.name && city.address && <div className="mt-2 p-3 bg-gradient-to-br from-amber-50 to-white rounded-lg border border-amber-200 shadow text-sm animate-in fade-in duration-300 w-full">
                                ...
                                {city.address && <div className="flex items-start mb-2 group hover:bg-amber-50/50 p-2 rounded-md transition-colors w-full">
                                ...
                                {city.contacts && city.contacts.length > 0 && <div className="flex items-start mb-2 group hover:bg-amber-50/50 p-2 rounded-md transition-colors w-full">
                                ...
                                {city.email && <div className="flex items-start group hover:bg-amber-50/50 p-2 rounded-md transition-colors w-full">
                                    <Mail className="w-4 h-4 mr-2 text-amber-500 mt-1 flex-shrink-0 group-hover:text-amber-600 transition-colors" />
                                    <a href={`mailto:${city.email}`} className="text-amber-600 hover:text-amber-800 hover:underline flex items-center text-sm break-words w-full overflow-hidden">
                                      {city.email}

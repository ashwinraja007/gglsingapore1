...
export const Header = () => {
  ...
  return <header className="rounded mx-0 my-0 py-0">
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <img src="/1GlobalEnterprises.png" alt="1 Global Enterprises Logo" className="h-8 md:h-10 w-auto object-contain" />
            </div>
            
            {/* Mobile Menu Button */}
            <button className="md:hidden text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-md p-1" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
          ...
          <div className={`${isMobileMenuOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'} md:hidden overflow-hidden transition-all duration-300 ease-in-out`}>
            <nav className="flex flex-col py-4 gap-4 border-t mt-4 border-gray-100">
              ...
              <Link to="/quote" className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-center font-medium shadow-sm hover:shadow w-full" onClick={() => setIsMobileMenuOpen(false)}>
                Get A Quote
              </Link>
            </nav>
          </div>

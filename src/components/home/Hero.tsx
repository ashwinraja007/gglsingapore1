    ...
    <section className="relative min-h-screen overflow-hidden pt-16">
      ...
      <div className="absolute top-0 right-0 z-50 p-4 md:hidden">
        <motion.button 
          ...
          whileTap={{ scale: 0.95 }}
          className="bg-brand-navy/80 backdrop-blur-md p-2 rounded-full shadow-lg border border-brand-gold/30 text-white"
                ...
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Link to="/services" onClick={() => setIsMobileMenuOpen(false)}>
                    <Button 
                      size="lg" 
                      variant="outline" 
                      className="w-full font-semibold shadow-lg hover:shadow-xl transition-all duration-300 text-white border-white hover:bg-white/20"
                    >
                      Our Services
                    </Button>
                  </Link>
            ...
            >
              ...
              >
                <Button 
                  variant="gold" 
                  size="lg" 
                  className="w-full sm:w-auto font-semibold shadow-lg hover:shadow-xl transition-all duration-300 gold-glow bg-[#d4af37] text-sm sm:text-base"
                >
                  Get A Quote
                </Button>
              </motion.a>
              ...
              >
                <Link to="/services">
                  <Button 
                    size="lg" 
                    variant="outline" 
                    className="w-full sm:w-auto font-semibold shadow-lg hover:shadow-xl transition-all duration-300 text-white border-white hover:bg-white/20 text-sm sm:text-base"
                  >
                    Our Services
                  </Button>
                </Link>
      ...
      >
        ...
        <motion.button 
          className="md:hidden w-full bg-brand-navy/90 backdrop-blur-md p-3 rounded-lg shadow-lg mb-3 flex items-center justify-between text-white border border-brand-gold/30" 
            ...
            >
              {portalLinks.map((link, index) => (
                ...
                >
                  <Link 
                    to={link.url} 
                    className="w-full p-3 flex items-center space-x-3 text-white"
                  >
                    <span className="text-brand-gold">{link.icon}</span>
        ...
        <div className="hidden md:grid grid-cols-3 lg:grid-cols-5 gap-2 lg:gap-3">
          {portalLinks.map((link, index) => (
            ...
            >
              <Link 
                to={link.url} 
                className="w-full h-full p-3 flex flex-col items-center justify-center text-white"
              >
                <motion.span 
                  className="text-brand-gold transform transition-transform p-2 bg-white/5 rounded-full mb-2"
                ...
                <div className="text-center">
                  <div className="font-medium text-sm">{link.title}</div>
                  <div className="text-xs text-white/70 mt-0.5 line-clamp-1">{link.description}</div>
                </div>
              </Link>
            </motion.div>
          ))}

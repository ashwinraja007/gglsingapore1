...
const Hero = () => {
  ...
  return <section className="relative min-h-screen overflow-hidden pt-8 md:pt-16">
        ...
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <div className={`max-w-2xl space-y-4 md:space-y-5 text-left transition-all duration-800 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2">
              <button className="w-full sm:w-auto bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-3 px-6 rounded-md shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                Get A Quote
              </button>
      ...
      {/* Portals Buttons (Bottom Section) - NO WHITE BORDER & FULLY RESPONSIVE */}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 sm:gap-3">
              {portalLinks.map((link, index) => <div key={index} className="flex flex-col items-center">
                  {link.external ? <a href={link.url} target="_blank" rel="noopener noreferrer" className="w-full">
                    ...
                    </a> : link.onClick ? <button className="w-full h-12 sm:h-14 md:h-16 flex flex-col gap-1 items-center justify-center text-xs sm:text-sm bg-white/90 hover:bg-white text-gray-800 hover:text-blue-900 transition-all duration-300 rounded-lg shadow-sm hover:shadow-md hover:scale-105" onClick={link.onClick}>
                      {link.icon}
                      <span className="font-medium leading-none">{link.title}</span>

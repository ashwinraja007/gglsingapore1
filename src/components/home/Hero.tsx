...
const Hero = () => {
  ...
  const sliderImages = ["/homeimage.jpg", "/transpoth.png", "/hero3.png", "/planeh2.png"];
    ...
    <section className="relative min-h-screen overflow-hidden pt-8 md:pt-16">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {sliderImages.map((image, index) => (
          <div
            key={index}
            className={`absolute inset-0 transition-opacity duration-1200 ease-in-out ${activeSlide === index ? 'opacity-100' : 'opacity-0'}`}
      ...
      <div className="absolute inset-0 flex items-center z-[2]">
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          <div className={`max-w-2xl space-y-4 md:space-y-5 text-left transition-all duration-800 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="hidden md:flex flex-col md:flex-row gap-3 pt-2">
              <button className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-3 px-6 rounded-md shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                Get A Quote
              </button>
              ...
              {portalLinks.map((link, index) => (
                <div key={index} className="flex flex-col items-center">
                  {link.external ? (
                    ...
                    <button onClick={link.onClick} className="w-full h-12 sm:h-14 md:h-16 flex flex-col gap-1 items-center justify-center text-xs sm:text-sm bg-white/90 hover:bg-white text-gray-800 hover:text-blue-900 transition-all duration-300 rounded-lg shadow-sm hover:shadow-md hover:scale-105">
                      {link.icon}
                      <span className="font-medium leading-none">{link.title}</span>
                    </button>
                  ) : (

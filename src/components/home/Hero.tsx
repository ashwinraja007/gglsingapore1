    ...
    <section className="relative min-h-screen overflow-hidden pt-16">
      ...
      <div className="fixed top-4 right-4 z-50 md:hidden">
        <motion.button 
          ...
          whileTap={{ scale: 0.95 }}
          className="bg-brand-navy/90 backdrop-blur-md p-3 rounded-full shadow-lg border border-brand-gold/50 text-white"
        ...
        <div className="grid grid-cols-3 lg:grid-cols-5 gap-3 lg:gap-4">
          {portalLinks.map((link, index) => (
            ...
            >
              <Link 
                to={link.url} 
                className="w-full h-full p-4 flex flex-col items-center justify-center text-white"
              >
                <motion.span 
                  className="text-brand-gold transform transition-transform p-2 bg-white/10 rounded-full mb-2" 
